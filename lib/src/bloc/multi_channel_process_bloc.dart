...
abstract class MultiChannelProcessBloc<TInput, TEvent extends ProcessBlocEvent, TState extends ProcessBlocState,
  ...
  MultiChannelProcessBloc(super.initialState) {
    on<TEvent>((event, emit) async {
      setBusy();
      try {
        final List<BlocChannel> applicableChannels = _channels.where((element) => element.isHandledBy(event)).toList();
